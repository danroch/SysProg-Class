...
int boot_server(char *ifaces, int port)
{
    ...
    // TO DO set up the socket - this is very similar to the demo code
...
int process_cli_requests(int svr_socket)
{
    ...
    while (1)
    {
        // TO DO use the accept syscall to create cli_socket
...
int exec_client_requests(int cli_socket)
{
    ...
    printf("Starting to handle client requests...\n");
...
int send_message_string(int cli_socket, char *buff)
{
    ...
    int message_len = strlen(buff);

    while (total_sent < message_len)
    {
        bytes_sent = send(cli_socket, buff + total_sent, message_len - total_sent, 0);
        if (bytes_sent <= 0)
        {
            perror("send");
            return ERR_RDSH_COMMUNICATION;
    ...
    // Send EOF to indicate the message is complete
 ...
 */
// exit
int rsh_cmd_exit(int cli_sock)
{
    dprintf(cli_sock, "Exiting shell\n");
    close(cli_sock);
    exit(EXIT_SUCCESS); // Exit the program
}

// cd
int rsh_cmd_cd(int cli_sock, command_list_t *clist)
{
...
// dragon
int rsh_cmd_dragon(int cli_sock)
{
...
// stop server
int rsh_cmd_stop_server(int cli_sock)
{
    dprintf(cli_sock, "Stopping server\n");
    close(cli_sock);
    exit(EXIT_SUCCESS);
}
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    int pids_st[num_commands];

    // build in commands to be handled by the parent
    ...
    if (cmd_type != BI_NOT_BI)
    {
        ...
        {
        case BI_CMD_EXIT:
            return rsh_cmd_exit(cli_sock);
        case BI_CMD_DRAGON:
            return rsh_cmd_dragon(cli_sock);
        case BI_CMD_CD:
            return rsh_cmd_cd(cli_sock, clist);
        case BI_CMD_STOP_SVR:
            return rsh_cmd_stop_server(cli_sock);
        }
    }
    ...
    for (int i = 0; i < num_commands; i++)
    {
        ...
        if (pids[i] == 0)
        {
            // read from cli_sock instead of stdin if first command
            if (i == 0)
            {
            ...
            // last command writes to cli_sock instead of stdout
            if (i == num_commands - 1)
            {
    ...
    exit_code = WEXITSTATUS(pids_st[num_commands - 1]);
    for (int i = 0; i < num_commands; i++)
    {
